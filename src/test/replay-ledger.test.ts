...
describe('Replay Ledger — Persistence', () => {
  ...
  describe('Hash Chain Integrity', () => {
    ...
    it('detects broken prev_event_hash', async () => {
      ...
      if (ledgerEvents[1]) {
        (ledgerEvents[1] as unknown as Record<string, unknown>).prev_event_hash = 'CORRUPTED_HASH';
      }
    ...
    it('detects broken event_hash', async () => {
      ...
      const ledgerEvents = listLedgerEventsInAppendOrder();
      if (ledgerEvents[1]) {
        (ledgerEvents[1] as unknown as Record<string, unknown>).event_hash = 'CORRUPTED_EVENT_HASH';
      }
  ...
  describe('Event Immutability', () => {
    ...
    it('prevents modification of frozen events', async () => {
      ...
      expect(() => {
        (appended as unknown as Record<string, unknown>).event_id = 'MODIFIED';
      }).toThrow();
    });
  });
